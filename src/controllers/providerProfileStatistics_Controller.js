...
router.get('/ventaSemana',(req,res) => {
    ...
    const sql = `select SUM(pedidos.total) as total from pedidos where created_at >= NOW()- INTERVAL 1 WEEK AND id_estado=5 and id in (select id_pedido from productos_pedidos where id_producto in (select id from productos where id_proveedor = ${globals.getID()}))`
    db.query(sql,(error,resultado)=>{
        if(error){
            console.error("Error"+error.message);
            return res.status(500).send("Error al consultar los datos");
